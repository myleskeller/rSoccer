...
class DefenderDeterministic:
    ...
    def decideObjective(self, ballPosition,ballSpeed, robotPos):
        dest = [0,0]
        p2 = (ballPosition[0] + ballSpeed[0]*5, ballPosition[1] + ballSpeed[1]*5)
        p1 = ballPosition

        angleBallGoal = math.atan2(Field.middle[1] - ballPosition[1], -ballPosition[0])
        angleBallToOurGoal = math.atan2(Field.middle[1] - ballPosition[1], Field.m_max[0]-ballPosition[0])

        ballDirection = math.atan2(p2[1]-p1[1],p2[0]-p1[0])
        
        dest = [ballPosition[0] + math.cos(ballDirection)*self.offensiveSafeRadius, ballPosition[1] + math.sin(ballDirection)*self.offensiveSafeRadius]
        
        triangle = [p1,(Field.goalAreaMax[0],Field.goalAreaMin[1]),Field.goalAreaMax]

        if (utils.PointInPolygon(triangle, p2)):
            distance = utils.euclideanDistance(ballPosition,robotPos)
            dest = [ballPosition[0] + math.cos(ballDirection)*distance,ballPosition[1] + math.sin(ballDirection)*distance]
        else:
            dest = [ballPosition[0] + math.cos(angleBallToOurGoal)*self.offensiveSafeRadius,ballPosition[1] + math.sin(angleBallToOurGoal)*self.offensiveSafeRadius]

        angle = math.atan2(dest[1]-ballPosition[1],dest[0]-ballPosition[0])
        auxPos = ((math.cos(angle) * 500) + ballPosition[0], (math.sin(angle) * 500) + ballPosition[1])
        auxPos2 = ((math.cos(angle+math.pi) * 500) + ballPosition[0], (math.sin(angle+math.pi) * 500) + ballPosition[1])
        ...
        if(utils.distancePointSegment(Field.goalMax,Field.goalMin,robotPos) > utils.distancePointSegment(Field.goalMax,Field.goalMin,ballPosition) and utils.distancePointSegment(auxPos,auxPos2,robotPos) < utils.halfAxis*4):
            ...
            dest = [(math.cos(angle) * self.offensiveSafeRadius) + ballPosition[0], (math.sin(angle) * self.offensiveSafeRadius) + ballPosition[1]]
        else:
