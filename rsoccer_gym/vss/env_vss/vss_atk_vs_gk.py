...
class VSSEnvATKVSGK(VSSBaseEnv):
    ...
    def __obs_gk(self):
        ...
        # se adiciona
        observation.append(self.norm_pos(-self.frame.robots_yellow[self.id_gk].x))
        observation.append(self.norm_pos(self.frame.robots_yellow[self.id_gk].y))
        observation.append(
            np.sin(np.deg2rad(self.frame.robots_yellow[self.id_gk].theta))
        )
        observation.append(
            -np.cos(np.deg2rad(self.frame.robots_yellow[self.id_gk].theta))
        )
        observation.append(self.norm_v(-self.frame.robots_yellow[self.id_gk].v_x))
        observation.append(self.norm_v(self.frame.robots_yellow[self.id_gk].v_y))
        observation.append(self.norm_w(-self.frame.robots_yellow[self.id_gk].v_theta))


        for i in range(self.n_robots_yellow):
            if i == self.id_gk: continue
    ...
    def _frame_to_observations(self):
        ...
        obs_gk = self.__obs_gk()
        return [np.array(observation, dtype=np.float32), obs_gk]
    ...
    def _get_commands(self, actions):
        ...
        actions_atk = actions[0]
        actions_gk = actions[1]
        v_wheel0, v_wheel1 = self._actions_to_v_wheels(actions_atk)
        ...
        for i in range(self.n_robots_yellow):
            if i == self.id_gk:
                v_wheel0, v_wheel1 = self._actions_to_v_wheels(actions_gk)
                commands.append(Robot(yellow=False, id=0, v_wheel0=v_wheel1,
                              v_wheel1=v_wheel0))
    ...
    def _get_initial_positions_frame(self):
        ...
        for i in range(self.n_robots_yellow):
            if i == self.id_gk:
                pos = (field_half_length - 0.05, 0.0)
