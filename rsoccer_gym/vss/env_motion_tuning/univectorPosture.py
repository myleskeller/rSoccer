...
class UnivectorPosture:
    def __init__(self):
        self.path=[]
    def calculateVectAngle(self, source, target, desiredPos):
    ...
    def getSurroundings(self, robotPos, ballPos, desiredPos):
        i = math.floor(robotPos[0])
        j = math.floor(robotPos[1])
        Nij = Ni1j = Ni1j1 = Nij1 =  N = aux = [0,0]
        da = db = dc = dd = 0.0
        ANij = self.calculateVectAngle((i, j), ballPos, desiredPos)
        Nij[0] = math.cos(ANij)
        Nij[1] = math.sin(ANij)
        ANi1j = self.calculateVectAngle((i + 1, j), ballPos, desiredPos)
        Ni1j[0] = math.cos(ANi1j)
        Ni1j[1] = math.sin(ANi1j)
        ANij1 = self.calculateVectAngle((i, j + 1), ballPos, desiredPos)
        Nij1[0] = math.cos(ANij1)
        Nij1[1] = math.sin(ANij1)
        ANi1j1 = self.calculateVectAngle((i + 1, j + 1), ballPos, desiredPos)
        ...
        N[0] = ((db * dc * dd * Nij[0]) + (da * dc * dd * Nij1[0]) + (da * db * dd * Ni1j[0]) + (da * db * dc * Ni1j1[0])) / (db * dc * dd + da * dc * dd + da * db * dd + da * db * dc)
        N[1] = ((db * dc * dd * Nij[1]) + (da * dc * dd * Nij1[1]) + (da * db * dd * Ni1j[1]) + (da * db * dc * Ni1j1[1])) / (db * dc * dd + da * dc * dd + da * db * dd + da * db * dc)
    ...
    def adjustToObstacle(self, source, direction, obstaclePosition):
        Ro = 10.0
        M = 4.0
        ...
        if (length < Ro + M and math.fabs(diff_angle) < math.pi / 2.0):
            if (distance <= Ro):
                ...
                direction = multiplier * math.fabs(math.atan((Ro + M) / math.sqrt(distance * distance + (Ro + M) * (Ro + M)))) + angle

        return direction

    def getVectorDirection(self,currentPosition, objectivePosition, robotPos, allies, enemies, index):
        direction = self.getSurroundings(currentPosition, objectivePosition, objectivePosition)

        for i in range(len(allies)):
            if (i == index):
                continue
            direction = self.adjustToObstacle(currentPosition, direction, allies[i])

        if(not utils.insideOurArea(robotPos,0,0)):
            for enemy in enemies:
                direction = self.adjustToObstacle(currentPosition, direction, enemy)

        return direction


    def update(self, ballPos, robotPos,objPos, allies, enemies, index):
        ...
        for _ in range(iterations):
            angle = self.getVectorDirection(curPos, objPos, robotPos, allies, enemies, index)
            ...
            if (utils.euclideanDistance(curPos, objPos) < 8): 
                ...
                while(utils.euclideanDistance(curPos, oldCurPos)<10.0):
                    curPos = [curPos[0] + math.cos(angle) * 5, curPos[1] + math.sin(angle) * 5]
            self.path.append(curPos)
            oldCurPos = curPos
        angle = self.getVectorDirection(robotPos, objPos, robotPos, allies, enemies, index)
